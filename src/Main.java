...
public class Main {
    ...
    @Test
    void testOfSerialization() {
        try {
            // Serialization
            FileOutputStream fileOut = new FileOutputStream("objects.ser");
            ObjectOutputStream out = new ObjectOutputStream(fileOut);

            // Tworzymy obiekty klas z projektu
            Klient klient1 = new Klient(0, "Patryk", "Kowalski", "address", "997");
            Lot lot1 = new Lot(0, "Krk", "WWA", "10-10-2023", "10:10", 269.0, 20, 6);
            Rezerwacja rezerwacja1 = new Rezerwacja(lot1, klient1, lot1.getDostepneMiejsca().get(0));

            //Wypiszmy je na terminal
            System.out.println("Wypiszmy zawartość obiektów przed serializacją korzystajac z metod toString(): \n");
            System.out.println(klient1.toString());
            System.out.println(lot1.toString());
            System.out.println(rezerwacja1.toString());

            // Serializujemy je do strumienia a strumien do pliku jw
            System.out.println("Serializujemy je do strumienia a strumien do pliku\n");
            out.writeObject(klient1);
            out.writeObject(lot1);
            out.writeObject(rezerwacja1);

            out.close();
            fileOut.close();

            System.out.println("Sserializacja udana. \n");

            // Deserialization
            FileInputStream fileIn = new FileInputStream("objects.ser");
            ObjectInputStream in = new ObjectInputStream(fileIn);

            // Tworzymy nowe obiekty inicjalizując je z pominięciem konstruktora
            // z wykorzystaniem mechanizmu deserializacji
            Klient klient2 = (Klient) in.readObject();
            Lot lot2 = (Lot) in.readObject();
            Rezerwacja rezerwacja2 = (Rezerwacja) in.readObject();

            in.close();
            fileIn.close();

            System.out.println("Deserializacja udana. \n");

            // Wypiszmy efekt pracy na ekran
            System.out.println("Wypiszmy zawartość obiektów stworzonych poprzez deserializację" +
                    " korzystajac z metod toString(): \n");
            System.out.println(klient2.toString());
            System.out.println(lot2.toString());
            System.out.println(rezerwacja2.toString());
